...
* Recall is treated as the primary optimization objective, bcause concluding that there isnt a fracture
* while there is, is medically speaking suboptimal. 

Note: Metrics are intended for model comparison and development

---

## Reproducibility & original data

The original dataset used in this project is the RSNA 2022 Cervical Spine Fracture Detection dataset (approximately 350GB).
This repository contains Stage 2 (classification) of a two-stage pipeline.

**Upstream Segmentation (Stage 1)**
Stage 2 assumes vertebra-level crops and their corresponding masks generated by a 3D segmentation model.
For this project, I used the publicly available Stage 1 segmentation outputs released by Qishen Ha (1st place solution, RSNA 2022 competition), which can be found here:
https://www.kaggle.com/datasets/haqishen/rsna-cropped-2d-224-0920-2m

The segmentation output was treated as upstream data and repackaged locally into vertebra-level tensors using scripts/pack_sequences.py.

This preprocessing step (pack_sequences):
* Converts the cropped segmentation outputs into packed .npy sequences
* Standardizes the input contract for Stage 2
* Reduces per-epoch training time
* The segmentation model itself is not reimplemented here and is treated as an upstream component.

Stage 2 expects pre-packed .npy files per vertebra:
* Filename: {StudyInstanceUID}_{c}.npy
* Shape: (T, H, W, 6)
* Channels:
